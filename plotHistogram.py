...
# # Go over each model and create frequency bar plots
# for model in models:
...
#     # Create frequency arrays for each category
#     frequencies = {category: [grouped_label_counts[i][label] for label in labels] for i, category in enumerate(categories)}
...
#         plt.bar(x + i * bar_width, frequencies[category], width=bar_width, label=category)

#     # Add labels and legend
#     plt.xticks(x + bar_width, labels, rotation=45, ha='right')  # Center the ticks between bars
#     plt.title(f'Frequency of Labels for {model}')
#     plt.xlabel('Labels')
