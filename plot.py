...
# # go over each model and do t-test
# for model in models:
#     # wrangle the data
#     en_only_scores = [entry['scores'][model] for entry in data if entry['mixed'] == 0]
#     cn_only_scores = [entry['scores'][model] for entry in data if entry['mixed'] == 1]
...
#     # do t-tests
#     t_stat_en_cn, p_value_en_cn = stats.ttest_ind(en_only_scores, cn_only_scores)
#     t_stat_en_mixed, p_value_en_mixed = stats.ttest_ind(en_only_scores, mixed_scores)
#     t_stat_cn_mixed, p_value_cn_mixed = stats.ttest_ind(cn_only_scores, mixed_scores)

#     print(f"Model: {model}")
#     print(f"T-statistic (EN vs CN): {t_stat_en_cn}, P-value: {p_value_en_cn}")
#     print(f"T-statistic (EN vs Mixed): {t_stat_en_mixed}, P-value: {p_value_en_mixed}")
#     print(f"T-statistic (CN vs Mixed): {t_stat_cn_mixed}, P-value: {p_value_cn_mixed}")

#     # plot data
#     plt.figure(figsize=(10, 6))
    
#     plt.hist(en_only_scores, alpha=0.5, label='EN Only', bins=10)
#     plt.hist(cn_only_scores, alpha=0.5, label='CN Only', bins=10)
#     plt.hist(mixed_scores, alpha=0.5, label='Mixed', bins=10)
